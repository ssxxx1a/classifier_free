...
def train(params):
    ...
    for epc in range(params.epoch):
        ...
        if (epc+1) % 5 == 0:
                torch.save(diffusion.model.state_dict(), os.path.join(params.moddir, f'ckpt_{epc+1}_diffusion.pt'))
                torch.save(cemblayer.state_dict(), os.path.join(params.moddir, f'ckpt_{epc+1}_cemblayer.pt'))
